# Installation, usage, and check details

See [README.md](README.md) for a short overview of what this tool does. This
document covers setup (Podman and Docker, compose or plain), the full API
reference, and a detailed explanation of every check the scanner runs.
...
See [README.md](README.md#example-output) for a full example response and
...
```json
{ "grade": "C", "score": 64, "pqc": "Hybrid PQC ready" }
```
...
Fail a CI step if the grade drops below B (note that an untrusted
certificate chain forces the grade to `"T"`, which also fails this check):
...
- The headline letter grade (A+ .. F, or T) is **not** affected by PQC
  results -- it's graded on classical protocol/cipher/certificate hygiene
  only. PQC readiness is surfaced as its own separate score/badge, since
  almost no server today negotiates a PQC hybrid group and that shouldn't
  tank an otherwise-good TLS configuration's grade.
